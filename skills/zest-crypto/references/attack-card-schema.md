...
become immutable tuples after parsing.
...
| Operator | Meaning |
| --- | --- |
| `exists` | the fact is present |
| `eq` / `neq` | exact typed equality or inequality |
| `lt` / `lte` / `gt` / `gte` | numeric comparison |
| `in` | the fact value occurs in the supplied typed candidate list |
| `contains` | a string or list fact contains the supplied typed value |
| `len_eq` / `len_gte` | a string or list length comparison |
...
cannot be absolute, use a parent component, or otherwise escape the skill root.
The parser validates containment but does not test file presence: packaged
template existence is a separate package-integrity check.
...
`required_tools`. Blocked and rejected entries retain the rule and evidence
reason that determined their state. The complete JSON below is parseable and
illustrates the public output shape.
...
`research-card-missing-pinned-example`. `invalid-card-id` is used for a card ID
outside the lowercase dotted/hyphenated grammar. Unknown fields, missing fields,
and type errors are also rejected rather than ignored.
